    ...
    public class RangedAttack : AttackStrategy
    {
        ...
        public override bool Attack(Vector2Int position, Vector2Int direction, bool isPlayerAttack)
        {
            ...
            foreach (var (target, damage) in lineTiles)
                ...
                {
                    Instantiate(projectilePrefab).GetComponent<ProjectileEffect>().LaunchFromTo(
                        MapManager.Instance.CellToWorld(position), MapManager.Instance.CellToWorld(target),
                        projectileTime);
                    return true;
                }
            ...
            Instantiate(projectilePrefab).GetComponent<ProjectileEffect>().LaunchFromTo(
                MapManager.Instance.CellToWorld(position), MapManager.Instance.CellToWorld(lastTarget.Item1),
                projectileTime);
            ...
            bool isPlayerAttack)
        {
            ...
            for (var i = 1; i <= range; i++)
            {
                ...
                if (blockedByActors && game.CellContainsActor(tilePosition))
