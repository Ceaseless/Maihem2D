...
namespace Maihem.Attacks
{
    public abstract class AttackStrategy : ScriptableObject
    {
        ...
        [Min(0)] [SerializeField] private int staminaCost;

        [Header("General Effects")] [SerializeField]
        private VisualEffectSettings attackVisualEffect;

        [SerializeField] private AudioClip attackSoundEffect;

        [Header("On Hit Effects")] [SerializeField]
        private VisualEffectSettings onHitVisualEffect;
        ...
        public abstract IList<(Vector2Int, int)> GetAffectedTiles(Vector2Int position, Vector2Int direction,
            bool isPlayerAttack);
        ...
        protected bool TryDamage(Vector2Int target, int adjustedDamage, bool isPlayerAttack)
        {
            ...
            if (!hitSomething)
            {
                if (attackVisualEffect is not null)
                    VisualEffectsPool.Instance.PlayVisualEffect(attackVisualEffect,
                        MapManager.Instance.CellToWorld(target));
            }
            else
            {
        ...
        private void PlayOnHitEffects(Vector2Int tile)
        {
            ...
            if (onHitSoundEffect is not null) AudioManager.Instance.PlaySoundFX(onHitSoundEffect, worldPosition, 1f);
        }
    }
}